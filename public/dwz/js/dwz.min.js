...
var DWZ = {
	...
	init:function(pageFrag, options){
		$("body").append('<div id="dialog"><a class="closedialog"></a><span class="title"></span><span class="content"></span><span class="time"></span></div><div id="showtip">查看上一条消息</div>');
        var showtip = $('#showtip');
        var dia = $('#dialog');
    ...
    tips: function(title, content) {
        ...
        if (m < 10) var m = '0' + m;
        if (s < 10) var s = '0' + s;
        var nowtime = t.getFullYear() + '-' + mon + '-' + day + ' ' + h + ':' + m + ':' + s;
        if (barheight) tipsheight += barheight + 1;
        dia.css('bottom', tipsheight + 'px').stop().hide();
        $('#dialog .time').html(nowtime);
        $('#dialog .title').html(title);
        $('#dialog .content').html(content);
        dia.animate({
            height: 'add.html'
        },
        1000).delay(DWZ.tipSet.longTime * 1000).animate({
            height: 'hide'
        },
        500,
        function() {
    ...
    ajaxtips: function() {
        if (DWZ.tipSet.tipType != 'ajax') return false;
        DWZ.runtips();
    },
    runtips: function() {
        $.ajax({
            type: 'post',
            url: DWZ.tipSet.ajaxUrl,
            dataType: 'json',
            global: false,
            success: function(d) {
                if (d) DWZ.tips(d.title, d.content)
            }
        });
    }
}; 
...
function initUI(_box){
	...
	$("textarea.kindeditor", $p).each(function () {
            ...
            if(!_tools){_tools='full';}
             ...
             if(_tools=='simple'){
                   items = ['fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold', 'italic', 'underline','removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright', 'insertorderedlist', 'insertunorderedlist', '|', 'image', 'link', 'fullscreen'];
             }else if(_tools=='full'){
                   items = ['source', '|', 'undo', 'redo', '|', 'preview', 'print', 'template', 'code', 'cut', 'copy', 'paste', 'plainpaste', 'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright','justifyfull', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript','superscript', 'clearhtml', 'quickformat', 'selectall', '|', 'fullscreen', '/','formatblock', 'fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold','italic', 'underline', 'strikethrough', 'lineheight', 'removeformat', '|', 'image', 'multiimage', 'flash', 'media', 'insertfile', 'table', 'hr', 'emoticons', 'baidumap','anchor', 'link', 'unlink'];
	...
	if ($.fn.navTabTodo) $("a[target=navTabTodo]", $p).navTabTodo();
	...
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			if ($table.attr("width") == "100%") {
				var th_w_all = 0;
				$table.children("thead").children("tr").find("th").each(function(i, n) {
					if ($(this).attr("width") != "") {
...
function iframeCallback(form, callback){
    ...
    if (!form.is_iframe) {
        $form.append('<input type="hidden" name="is_iframe" value="1" />');
    }
...
function ajaxTodoDel(url, callback){
	$.ajax({
		type:'POST',
		url:url,
		dataType:"json",
		...
		return this.each(function(){
			...
			$this.click(function(event){
				...
				if (title) {
					...
					ajaxTodoDel(url, $this.attr("callback"));
				}
				event.preventDefault();
			});
		});
